        ...
        img.onload = function(){
            ...
            for (let i = 0; i < textData.data.length; i += 4) {
                if (textData.data[i+3] !== 0) { //zwart in geheime tekst
                    if (imgData.data[i+2] % 10 == 3) {
                        //doe niets
                    }
                    else if (imgData.data[i+2] > 253) {
                        imgData.data[i+2] = 253; //begrens de waarde
                    }
                    else {
                        while (imgData.data[i+2] % 10 != 3) {
                            imgData.data[i+2]++;//zorg dat op 3 eindigt
                        }
                    }
                    pixelsInMsg++;
                }
